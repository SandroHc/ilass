...
impl VideoDecoder {
    ...
    pub(crate) fn decode<T>(
        ...
        let mut format_context =
            format::input(&file_path).with_context(|_| DecoderErrorKind::OpenFile(file_path.as_ref().to_path_buf()))?;
        ...
        let mut process_frames = |decoder: &mut decoder::Audio| -> Result<(), DecoderError> {
            ...
            while decoder.receive_frame(&mut decoded).is_ok() {
                ...
                resampler
                    .run(&decoded, &mut resampled_frame)
                    .with_context(|_| DecoderErrorKind::Decode)?;

                // Extract all samples from channel 0 (mono)
                let sample_count = resampled_frame.samples();
                let samples =
                    unsafe { std::slice::from_raw_parts(resampled_frame.data(0).as_ptr() as *const i16, sample_count) };
                receiver
